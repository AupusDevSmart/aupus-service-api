...
export class AnexosOSService {
  ...
  async uploadAnexo(
    ...
    await this.verificarOSExiste(osId);
    ...
    const anexo = await this.prisma.anexos_os.create({
      data: {
        nome: nomeArquivo,
        ...
        tamanho: file.size,
        descricao,
        caminho_s3: caminhoLocal,
        url_download: `/anexos-os/${nomeArquivo}`,
        fase_execucao: faseExecucao,
        uploaded_by_id: usuarioId,
        ordem_servico: { connect: { id: osId } },
      },
    });
  ...
  async listarAnexosOS(osId: string): Promise<AnexoOSResponseDto[]> {
    await this.verificarOSExiste(osId);

    const anexos = await this.prisma.anexos_os.findMany({
      where: {
        os_id: osId,
        deletado_em: null,
      },
  ...
  async buscarAnexo(anexoId: string): Promise<AnexoOSResponseDto> {
    const anexo = await this.prisma.anexos_os.findFirst({
      where: {
        id: anexoId,
        deletado_em: null,
      },
  ...
  async removerAnexo(anexoId: string, usuarioId?: string): Promise<void> {
    ...
    await this.prisma.anexos_os.update({
      where: { id: anexoId },
  ...
  private async verificarOSExiste(osId: string): Promise<void> {
  ...
  private async salvarArquivoLocal(file: any, nomeArquivo: string): Promise<string> {
    const uploadDir = path.join(process.cwd(), 'uploads', 'anexos-os');
  ...
  private mapearParaResponse(anexo: any): AnexoOSResponseDto {
    return {
      id: anexo.id,
      os_id: anexo.os_id,
      nome: anexo.nome,
      ...
      fase_execucao: anexo.fase_execucao,
      uploaded_at: anexo.uploaded_at,
      uploaded_by: anexo.uploaded_by,
      uploaded_by_id: anexo.uploaded_by_id,
      deletado_em: anexo.deletado_em,
    };
  }
}